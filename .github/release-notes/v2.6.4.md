...
Bug fixes, a smaller CIA, and better hardware compatibility: HOME button crashes in debug builds fixed, CIA build ~32 MB smaller with gallery covers working, nine EPUB fixes for CSS font sizes, multi-stylesheet books, large anthology indexes, the Clear Cache button, inline link navigation controls, block element vertical margins, transparent image compositing, text-align overflow, and CSS margin-top/bottom on block elements. Also includes EPUB rendering improvements: transparent decorative images now composite correctly, table-cell images no longer produce text artifacts, superscript/subscript now uses native FreeType rendering at 70% size, CSS text-indent is supported, list items with inner paragraphs render correctly, centered text no longer appears right-shifted on the second screen, and continuation wrap lines no longer appear shifted on the second screen. PDF/CBZ maximum zoom is now unified at 4.0× on both Old 3DS and New 3DS. The in-book settings menu now exposes font, size, spacing, and orientation controls, and returning from settings automatically reopens the book when layout-affecting options changed.
...
* #91: in-book settings (Select) now shows font, size, spacing, orientation, and publisher font-size controls; exiting automatically reopens the book when any layout-affecting setting changed
